...
- **Command Timing**: Automatic timing tracking for all commands
  - **Duration Display**: Shift+Click any marker to see execution time (e.g., "⏱️ 1.23s")
...
Shift+Clicking a command marker opens a context menu with options:
...
- **Focused Components** - TabBar, TerminalGrid, AppToolbar, WindowRouter, CommandPalette
...
For developers interested in contributing, see [DEV_GUIDE.md](DEV_GUIDE.md).

### Modular Design

The refactoring reduced the main `App.tsx` significantly by extracting:

- **State Management**: 10 custom hooks handle tabs, sessions, SSH, commands, keyboard shortcuts
- **UI Components**: TabBar, AppToolbar, TerminalGrid, WindowRouter, CommandPalette for focused rendering
